...
import { getFirestore, collection, addDoc, onSnapshot, doc, query, getDocs, updateDoc, deleteDoc, serverTimestamp } from 'firebase/firestore';
import { getAuth, signInAnonymously, onAuthStateChanged } from 'firebase/auth';

// --- Configuration ---
// This is a unique identifier for your app's data path in the database.
...
export default function App() {
        ...
        try {
            ...
            const unsubscribe = onAuthStateChanged(authInstance, async (currentUser) => {
                if (currentUser) { 
                    ...
                    // Sign in users anonymously for a consistent session.
    ...
    useEffect(() => {
        ...
        const unsubTasks = onSnapshot(query(collection(db, `artifacts/${appId}/public/data/tasks`)), (snapshot) => {
            ...
            setIsLoading(false);
        }, (err) => { console.error(err); setError("Failed to fetch tasks."); setIsLoading(false); });
    ...
    const TaskModal = ({ show, onClose, taskToEdit }) => {
        if (!show) return null;
    ...
    const SettingsModal = ({ show, onClose }) => {
        if (!show) return null;
        const [newItemName, setNewItemName] = useState(''); const [newClientName, setNewClientName] = useState('');
        const handleAddItem = async (type) => { const name = type === 'team' ? newItemName : newClientName; if (!name.trim() || !db) return; await addDoc(collection(db, `artifacts/${appId}/public/data/${type}`), { name: name.trim() }); if (type === 'team') setNewItemName(''); else setNewClientName(''); };
        const handleDeleteItem = async (type, id) => { if (!db) return; await deleteDoc(doc(db, `artifacts/${appId}/public/data/${type}`, id)); };
        return (
    ...
    return (
        <div className="bg-[#0D0D0D] text-white min-h-screen font-sans">
            <div className="container mx-auto p-4 sm:p-6 lg:p-8">
                ...
                {!isLoading && !error && (currentView === 'board' ? <KanbanView /> : <StandupView />)}
            </div>
            <TaskModal show={showTaskModal} onClose={handleCloseTaskModal} taskToEdit={editingTask} />
            ...
            <footer className="text-center p-4 text-xs text-gray-600"><p>User ID: {user ? user.uid : 'Not signed in'} | App ID: {appId}</p></footer>
        </div>
    );
}
